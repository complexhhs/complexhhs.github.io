...
$$ p(f(x)|f(x_1),f(x_2),f(x_3),...f(x_n)) $$. (9)

식 (9)는 주어진 데이터로 조건부 확률 공식으로써 $f(x)$ 및 $f(x_i), \quad \forall{i}$가 각각 독립사건으로 보면 다음과 같이 전개된다. 

$$\begin{align} p(f(x)|f(x_1),f(x_2),...f(x_n)) &= \frac{p\left(f(x), f(x_1), f(x_2)...f(x_n) \right)}{p\left(f(x_1),f(x_2)...f(x_n) \right)} \\
&=\frac{\mathcal{N}\left(f(x), f(x_1), f(x_2)...f(x_n) |0,\tilde{C}  \right)}{\mathcal{N}\left( f(x_1), f(x_2)...f(x_n) |0,C  \right)} \\
&=\mathcal{N}\left(f(x)|{\mu},{\sigma}^2   \right) \end{align}.$$ (10)
